...
def search_one(query: str, top_k: int = 5, base_url: str = "http://localhost:18088") -> tuple[list[str], float]:
    """
    调一次 search 接口,返回 (returned_ids, elapsed_ms)
    - returned_ids: chunks[i].id 列表,长度等于 top_k(除非接口返回少)
    - elapsed_ms: 这次请求的耗时(毫秒)
    异常处理:
    - 网络异常 / 非 200 状态码 → 抛 RuntimeError,带具体错误信息
    """
...
if __name__ == "__main__":
    ...
    report_path = str(_here / "reports" / f"eval_{timestamp}")

    print(f"Loading testset from {testset_path}...")
    testset = load_testset(testset_path)
    print(f"Loaded {len(testset)} cases.")

    print("Running Eval (this may take a while)...")
